...
static ssize_t local_write(const struct iio_device *dev, const void *src,
			   size_t len)
{
	struct iio_device_pdata *pdata = dev->pdata;
					...
					size_t len, enum iio_attr_type type)
{
	...
	for (i = 0; len >= 4 && i < nb; i++) {
		/* Recursive! */
		ssize_t ret = local_read_dev_attr(dev, attrs[i], ptr + 4,
						  len - 4, type);
					...
					char *dst, size_t len)
{
	...
	for (i = 0; len >= 4 && i < chn->nb_attrs; i++) {
		/* Recursive! */
		ssize_t ret = local_read_chn_attr(chn, chn->attrs[i].name,
						  ptr + 4, len - 4);
		*(uint32_t *)ptr = iio_htobe32(ret);
...
static int channel_write_state(const struct iio_channel *chn, bool en)
{
	...
	ret = local_write_chn_attr(chn, chn->pdata->enable_fn, en ? "1" : "0",
				   2);
	if (ret < 0)
		return (int)ret;
	else
		      ...
		      bool cyclic)
{
	...
	ret = local_write_dev_attr(dev, "buffer/length", buf, strlen(buf) + 1,
				   false);
	if (ret < 0)
		return ret;
...
static int handle_scan_elements(struct iio_channel *chn)
{
	...
	for (i = 0; i < pdata->nb_protected_attrs; i++) {
		int ret = handle_protected_scan_element_attr(
				chn, pdata->protected_attrs[i].name,
...
{
	struct iio_device **devices = realloc(
			ctx->devices,
...
static unsigned int is_global_attr(struct iio_channel *chn, const char *attr)
{
	...
	if (dashptr && dashptr > attr && dashptr < ptr) {
		...
		if (iddashptr && strlen(iddashptr + 1) > len2 &&
		    iddashptr - chn->id > len1 && chn->id[len1] >= '0' &&
		    chn->id[len1] <= '9' && !strncmp(chn->id, attr, len1) &&
