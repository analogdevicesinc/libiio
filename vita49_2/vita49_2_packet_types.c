...
__vrt_api ssize_t vita49_2_serialize_context_packet(struct vita49_2_context_packet* const pkt, uint32_t* const buf, size_t max_words)
{
	...
	buf[buffer_index++] = htonl(pkt->cif0.word);

	// CIF1 Word
	if (pkt->cif0.cif1_enable && pkt->cif1 != NULL)
	{
		if (buffer_index >= max_words)
			return -ENOBUFS;

		buf[buffer_index++] = htonl(pkt->cif1->word);
	}

	// CIF2 Word
	if (pkt->cif0.cif2_enable && pkt->cif2 != NULL)
	{
		if (buffer_index >= max_words)
			return -ENOBUFS;

		buf[buffer_index++] = htonl(pkt->cif2->word);
	}

	// CIF3 Word
	if (pkt->cif0.cif3_enable && pkt->cif3 != NULL)
	{
		if (buffer_index >= max_words)
			return -ENOBUFS;

		buf[buffer_index++] = htonl(pkt->cif3->word);
	}

	// CIF7 Word
	if (pkt->cif0.cif7_enable && pkt->cif7 != NULL)
	{
		if (buffer_index >= max_words)
			return -ENOBUFS;

		buf[buffer_index++] = htonl(pkt->cif7->word);
	}

	// CIF0 Attribute Data
	if ((buffer_index = vita49_2_serialize_cif0_attributes(&pkt->cif0, buf, buffer_index, max_words, false)) < 0)
		return buffer_index;
...
__vrt_api ssize_t vita49_2_serialize_control_packet(struct vita49_2_control_packet* const pkt, uint32_t* const buf, size_t max_words)
{
	...
	// Common Prologue
	if ((buffer_index = vita49_2_serialize_common_prologue(&pkt->command_prologue.common_prologue, buf, max_words)) < 0)
		return buffer_index;
	...
	buf[buffer_index++] = htonl(pkt->command_prologue.control_cam->word);	
	...
	// CIF0 Attribute Data
	if ((buffer_index = vita49_2_serialize_cif0_attributes(&pkt->cif0, buf, buffer_index, max_words, true)) < 0)
		return buffer_index;
...
__vrt_api ssize_t vita49_2_serialize_ackX_packet(struct vita49_2_ackX_packet* const pkt, uint32_t* const buf, size_t max_words)
{
	...
	buf[buffer_index++] = htonl(pkt->command_prologue.ack_cam->word);	
...
__vrt_api ssize_t vita49_2_serialize_ackV_packet(const struct vita49_2_ackV_packet* const pkt, uint32_t* const buf, size_t max_words)
{
	if (pkt == NULL || buf == NULL || max_words == 0)
		return -EINVAL;
...
__vrt_api ssize_t vita49_2_serialize_ackS_packet(const struct vita49_2_ackS_packet* const pkt, uint32_t* const buf, size_t max_words)
{
	if (!pkt || !buf)
		return -EINVAL;
...
__vrt_api ssize_t vita49_2_serialize_control_extension_packet(const struct vita49_2_control_extension_packet* const pkt, uint32_t* const buf, size_t max_words)
{
	if (!pkt || !buf)
		return -EINVAL;
...
__vrt_api int vita49_2_parse_control_extension_packet(const uint32_t* const buf, size_t buf_words, struct vita49_2_control_extension_packet* const pkt)
{
	...
	{
		if (pkt->command_prologue.common_prologue.class_id.lower_word.pad_bit_count % 8 != 0)
		{
