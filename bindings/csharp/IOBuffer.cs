...
class IOBuffer : IDisposable {
    private
	bool circular_buffer_pushed;

	[DllImport("libiio.dll",
			CallingConvention =
					CallingConvention.Cdecl)] private static extern IntPtr
	iio_device_create_buffer(IntPtr dev, uint samples_count,
			[MarshalAs(UnmanagedType.I1)] bool circular);

	[DllImport("libiio.dll",
			CallingConvention =
					CallingConvention.Cdecl)] private static extern void
	iio_buffer_destroy(IntPtr buf);

	[DllImport("libiio.dll",
			CallingConvention =
					CallingConvention.Cdecl)] private static extern int
	iio_buffer_refill(IntPtr buf);

	[DllImport("libiio.dll",
			CallingConvention =
					CallingConvention.Cdecl)] private static extern int
	iio_buffer_push_partial(IntPtr buf, uint samples_count);

	[DllImport("libiio.dll",
			CallingConvention =
					CallingConvention.Cdecl)] private static extern IntPtr
	iio_buffer_start(IntPtr buf);

	[DllImport("libiio.dll",
			CallingConvention =
					CallingConvention.Cdecl)] private static extern IntPtr
	iio_buffer_end(IntPtr buf);
	...
	IOBuffer(Device dev, uint samples_count, bool circular = false)
	{
		...
		buf = iio_device_create_buffer(
				dev.dev, samples_count, circular);
		if (buf == IntPtr.Zero)
	...
	void refill()
	{
		...
		if (err < 0)
			throw new Exception(
					"Unable to refill buffer: err=" + err);
	}
	...
	{
		long length = (long)iio_buffer_end(buf) -
			      (long)iio_buffer_start(buf);
