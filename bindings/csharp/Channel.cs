...
class Channel {
	...
	class ChannelAttr : Attr {
		...
		[DllImport("libiio.dll",
			   CallingConvention =
					   CallingConvention
							   .Cdecl)] private static extern int
		iio_channel_attr_write(IntPtr chn, [In()] string name,
				       string val);
	...
	byte[] read(IOBuffer buffer, bool raw = false)
	{
		...
		byte[] array = new byte[(int)(buffer.samples_count *
					      sample_size)];
		...
		if (raw)
			count = iio_channel_read_raw(
					this.chn, buffer.buf, addr,
