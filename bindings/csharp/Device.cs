...
class Device {
	...
	class DeviceAttr : Attr {
	    ...
	    public
		DeviceAttr(IntPtr dev, string name)
			: base(name)
		{
			this.dev = dev;
		}
		...
		override string read()
		{
			...
			int err = iio_device_attr_read(
					dev, name, builder, 1024);
			if (err < 0)
				throw new Exception(
	...
	private class DeviceDebugAttr : Attr {
	    ...
	    public
		DeviceDebugAttr(IntPtr dev, string name)
			: base(name)
		{
			this.dev = dev;
		}
		...
		{
			StringBuilder builder = new StringBuilder(1024);
			int err = iio_device_debug_attr_read(
					dev, name, builder, 1024);
			if (err < 0)
				throw new Exception(
	...
	internal Device(Context ctx, IntPtr dev)
	{
		...
		for (uint i = 0; i < nb_attrs; i++)
			attrs.Add(new DeviceAttr(dev,
					Marshal.PtrToStringAnsi(
							iio_device_get_attr(dev,
									i))));
		for (uint i = 0; i < nb_debug_attrs; i++)
			debug_attrs.Add(new DeviceDebugAttr(dev,
					Marshal.PtrToStringAnsi(
	...
	void set_trigger(Trigger trig)
	{
		int err = iio_device_set_trigger(this.dev,
				trig == null ? IntPtr.Zero : trig.dev);
		if (err < 0)
			throw new Exception(
					"Unable to set trigger: err=" + err);
	}
