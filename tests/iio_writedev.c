...
static ssize_t read_sample(const struct iio_channel *chn, void *buf, size_t len,
			   void *d)
{
...
int main(int argc, char **argv)
{
	...
	if (timeout >= 0) {
		...
		if (ret < 0) {
			...
			iio_strerror(-ret, err_str, sizeof(err_str));
			fprintf(stderr,
				"IIO contexts set timeout failed : %s (%zd)\n",
				err_str, ret);
		}
	}
	...
	while (app_running) {
		...
		if (iio_buffer_step(buffer) == sample_size) {
			...
			ret = iio_buffer_foreach_sample(buffer, read_sample,
							NULL);
			if (ret < 0) {
				char buf[256];
				iio_strerror(-ret, buf, sizeof(buf));
				fprintf(stderr,
					"buffer processing failed : %s (%zd)\n",
					buf, ret);
			}
		}
