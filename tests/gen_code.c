...
void gen_start(const char *gen_file)
{
	...
	if (lang == C_LANG) {
		...
		fprintf(fd,
			"/* These macros are for illustrative purposes only */\n");
		fprintf(fd, "#define IIO_ASSERT(expr) { \\\n");
		fprintf(fd, "\tif (!(expr)) { \\\n");
		fprintf(fd, "\t\tiio_strerror(errno, buf, sizeof(buf)); \\\n");
		fprintf(fd,
			"\t\t(void) fprintf(stderr, \"Assertion triggered:\\n\"); \\\n");
		fprintf(fd,
...
void gen_context_attr(const char *key)
{
	...
	if (lang == C_LANG) {
		fprintf(fd,
			"\t/* Read IIO Context attribute and return result as string */\n");
		fprintf(fd,
			"\tval_str = iio_context_get_attr_value(ctx, \"%s\");\n",
			key);
...
void gen_dev(const struct iio_device *dev)
{
	...
	if (lang == C_LANG) {
		...
		fprintf(fd,
			"\tIIO_ASSERT(dev = iio_context_find_device(ctx, \"%s\"));\n\n",
...
void gen_ch(const struct iio_channel *ch)
{
	...
	if (lang == C_LANG) {
		fprintf(fd,
			"\t/* Find the IIO %s channel in the current device */\n",
			iio_channel_is_output(ch) ? "Output" : "Input");
		fprintf(fd,
			"\tIIO_ASSERT(ch = iio_device_find_channel(dev, \"%s\", %s));\n\n",
		...
		fprintf(fd, "    ch = dev.find_channel('%s', %s)\n", name,
			iio_channel_is_output(ch) ? "True" : "False");
	}
}
		  ...
		  const char *wbuf)
{
	...
	if (lang == C_LANG) {
		if (wbuf) {
			fprintf(fd,
				"\t/* Write null terminated string to %s attribute: */\n",
				prefix);
			fprintf(fd,
				"\tRET_ASSERT(ret = iio_%s_attr_write(\n"
				"\t\t\t%s, \"%s\", \"%s\"));\n",
				prefix, target, attr, wbuf);
		} else {
			fprintf(fd,
				"\t/* Read IIO %s attribute, and put result in string */\n",
				prefix);
			fprintf(fd,
				"\tRET_ASSERT(ret = iio_%s_attr_read(\n"
				"\t\t\t%s, \"%s\", buf, sizeof(buf)));\n",
				prefix, target, attr);
		}
		fprintf(fd, "\t/* For other types, use:\n");
		fprintf(fd,
			"\t *  ret = iio_%s_attr_%s_bool(%s, \"%s\", v_bool);\n",
			prefix, rw, target, attr);
		fprintf(fd,
			"\t *  ret = iio_%s_attr_%s_double(%s, \"%s\", v_double);\n",
			prefix, rw, target, attr);
		fprintf(fd,
			"\t *  ret = iio_%s_attr_%s_longlong(%s, \"%s\", v_ll);\n",
			prefix, rw, target, attr);
		fprintf(fd,
			"\t *******************************************************************/\n");
		if (wbuf) {
		...
		if (wbuf) {
			fprintf(fd, "    # Write string to %s attribute:\n",
				prefix);
			if (!strcmp(prefix, "device") ||
			    !strcmp(prefix, "channel")) {
				fprintf(fd,
					"    %s.attrs[\"%s\"].value = str(\"%s\")\n",
					target, attr, wbuf);
			} else if (!strcmp(prefix, "device_debug")) {
				fprintf(fd,
					"    %s.debug_attrs[\"%s\"].value = str(\"%s\")\n",
					target, attr, wbuf);
			} else {
				fprintf(fd,
					"    # Write for %s / %s not implmented yet\n",
					prefix, target);
			}
			...
			if (!strcmp(prefix, "device") ||
			    !strcmp(prefix, "channel")) {
				fprintf(fd,
					"    print(\"%s : \" + %s.attrs[\"%s\"].value)\n",
					attr, target, attr);
			} else if (!strcmp(prefix, "device_debug")) {
				fprintf(fd,
					"    print(\"%s : \" + %s.debug_attrs[\"%s\"].value)\n",
					attr, target, attr);
			} else {
				fprintf(fd,
					"    # Read for %s / %s not implmented yet\n",
					prefix, target);
			}
		}
	}
