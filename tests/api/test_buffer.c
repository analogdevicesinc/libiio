...
TEST_FUNCTION(buffer_creation_basic)
{
	...
	struct iio_buffer *buffer = iio_device_create_buffer(test_dev, NULL, mask);
	if (iio_err(buffer)) {
...
TEST_FUNCTION(buffer_creation_with_params)
{
	setup_test_buffer();
	...
	struct iio_buffer_params params = {
		.idx = 0,
		.dma_allocator = IIO_DMA_ALLOCATOR_SYSTEM,
	};

	struct iio_buffer *buffer = iio_device_create_buffer(test_dev, &params, mask);
	if (iio_err(buffer)) {
		DEBUG_PRINT("  INFO: Buffer creation with params failed with error %d\n", iio_err(buffer));
		iio_channels_mask_destroy(mask);
		return;
	}

	TEST_ASSERT_PTR_NOT_NULL(buffer, "Buffer with params should be created");
...
int main(void)
{
	...
	RUN_TEST(buffer_creation_with_params);
