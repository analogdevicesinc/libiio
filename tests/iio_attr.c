...
int main(int argc, char **argv)
{
	...
	if ((gen_code || wbuf) &&
			((device_index &&
					 (!strcmp(".", argv[device_index]) ||
							 strchr(argv[device_index],
									 '*'))) ||
					(channel_index &&
							(!strcmp(".", argv[channel_index]) ||
	...
	if (search_device || search_channel || search_buffer || search_debug) {
		...
		for (i = 0; i < nb_devices; i++) {
			...
			if (device_index && !str_match(name, argv[device_index],
							    ignore_case))
				continue;
			...
			for (j = 0; j < nb_channels; j++) {
				...
				if (output_only && !iio_channel_is_output(ch))
					continue;
				if (scan_only && !iio_channel_is_scan_element(
								 ch))
					continue;
				...
				if (channel_index &&
						!str_match(iio_channel_get_id(
									   ch),
								argv[channel_index],
								ignore_case) &&
						(!name || (name && !str_match(name,
										   argv[channel_index],
										   ignore_case))))
					continue;
			...
			if (search_debug && device_index && nb_attrs) {
				...
				for (j = 0; j < nb_attrs; j++) {
					...
					if ((attr_index && str_match(attr, argv[attr_index],
							...
							!attr_index) {
						gen_dev(dev);
						dump_debug_attributes(dev, attr,
								wbuf,
								attr_index ? quiet
									   : false);
					}
