...
static ssize_t iiod_client_read_integer(struct iiod_client *client, void *desc,
					int *val)
{
	unsigned int i;
	...
	do {
		ret = client->ops->read_line(client->pdata, desc, buf,
					     sizeof(buf));
		if (ret < 0) {
			ERROR("READ LINE: %zd\n", ret);
			return ret;
...
static int iiod_client_exec_command(struct iiod_client *client, void *desc,
				    const char *cmd)
{
	int resp;
	ssize_t ret;
...
static ssize_t iiod_client_read_all(struct iiod_client *client, void *desc,
				    void *dst, size_t len)
{
	struct iio_context_pdata *pdata = client->pdata;
	const struct iiod_client_ops *ops = client->ops;
...
int iiod_client_set_timeout(struct iiod_client *client, void *desc,
...
ssize_t iiod_client_read_attr(struct iiod_client *client, void *desc,
			      ...
			      char *dest, size_t len, enum iio_attr_type type)
{
...
ssize_t iiod_client_read_unlocked(struct iiod_client *client, void *desc,
				  const struct iio_device *dev, void *dst,
