...
void iiod_client_free_block(struct iio_block_pdata *block)
{
	...
	cmd.dev = (uint8_t) iio_device_get_index(pdata->dev);
	cmd.code = pdata->idx | (pbuffer->idx << 16);
			    ...
			    struct iio_block_pdata *block)
{
	...
	shared.buf_idx = default_buf->buffer->idx;
	...
	printf("share block (idx=%d) from %s to %s, idx=%dn buf_idx=%d\n", default_buf->idx,
	       iio_device_get_name(default_buf->buffer->dev),
	       iio_device_get_name(pdata->dev), pbuf->idx, pdata->idx);
...
{
	for (unsigned int i = 0; i < block->n_bufs; i++)
			       ...
			       struct iio_block_pdata *block)
{
	...
	cmd.dev = (uint8_t) iio_device_get_index(pdata->dev);
	cmd.code = pdata->idx | (pbuf->idx << 16);
	printf("Unshare block (idx=%d) from %s\n", pbuf->idx,
	       iio_device_get_name(pdata->dev));
				       ...
				       struct iio_block_pdata *block, bool nonblock)
{
	...
	if (ret < 0) {
		...
		/* Enqueueing succeeded, but dequeueing failed. Return the
