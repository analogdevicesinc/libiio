...
build_libiio() {
    ...
    $CMAKE -G "MinGW Makefiles" -DPYTHON_EXECUTABLE:FILEPATH=$(python -c "import os, sys; print(os.path.join(os.path.dirname(sys.executable), 'python.exe')") \
        -DCMAKE_SYSTEM_PREFIX_PATH="C:" -DCMAKE_BUILD_TYPE="$build_type" -Werror=dev -DCOMPILE_WARNING_AS_ERROR=ON \
        -DENABLE_IPV6=ON -DWITH_USB_BACKEND=ON -DWITH_SERIAL_BACKEND=ON -DWITH_EMU_BACKEND=ON -DWITH_IIOD_EMU=ON -DPYTHON_BINDINGS=ON -DCPP_BINDINGS=ON \
