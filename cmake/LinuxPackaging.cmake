...
elseif (distro MATCHES "\\.*SUSE\\.*")
    ...
    message(
        STATUS "found unknown distribution ${distro}, Version : ${disversion}")
    message(
        FATAL_ERROR
            "please report an error to https:\\/\\/github.com\\/analogdevicesinc\\/libiio\\/issues"
    )
endif ()
...
if (CPACK_GENERATOR MATCHES "DEB")
    ...
    set(CPACK_PACKAGE_CONTACT
        "Engineerzone <https://ez.analog.com/community/linux-device-drivers>")
endif ()
...
if (NOT CMAKE_CROSSCOMPILING)
    ...
    message(
        FATAL_ERROR
            "package building with cross compile not handled - please -DENABLE_PACKAGING=OFF"
    )
endif ()

# don't add a package dependency if it is not installed locally these should be
...
if (HAVE_AVAHI)
    if (CPACK_GENERATOR MATCHES "DEB")
        ...
        set(DEFAULT_DEB
            "${DEFAULT_DEB}libavahi-client3 (>= 0.6.31), libavahi-common3 (>= 0.6.31), "
        )
    elseif (CPACK_GENERATOR MATCHES "RPM")
...
# find the version of the installed package, which is hard to do in cmake first,
# turn the list into a list (separated by semicolons)
string(REGEX REPLACE " " ";" PACKAGES ${PACKAGES})
# then iterate over each
foreach (package ${PACKAGES})
    # List packages matching given pattern ${package}, key is the glob (*) at
    # the end of the ${package} name, so we don't need to be so specific
    if (CPACK_GENERATOR MATCHES "DEB" AND DPKG_CMD)
        execute_process(COMMAND "${DPKG_CMD}" -l ${package}*
                        OUTPUT_VARIABLE DPKG_PACKAGES)
        # returns a string, in a format: ii  libxml2:amd64  2.9.4+dfsg1- amd64
        # GNOME XML library 'ii' means installed - which is what we are looking
        # for
        string(
            REGEX
                MATCHALL
                "ii  ${package}[a-z0-9A-Z.-]*:${CPACK_DEBIAN_PACKAGE_ARCHITECTURE}"
                DPKG_INSTALLED_PACKAGES
                ${DPKG_PACKAGES})
        # get rid of the 'ii', and split things up, so we can look at the name
    ...
    foreach (match ${NAME_INSTALLED_PACKAGES})
        # ignore packages marked as dev, debug, documentations, utils, data or
        # cross
        string(
            REGEX
                MATCHALL
                "-dev|-dbg|-doc|-lang|-extra|-data|-utils|-cross|-plugins|-python|-headers|-gobject|-locale|-tools|glibc-common|libusbmuxd|libusbx"
                TEMP_TEST
                ${match})
        ...
        if (CPACK_GENERATOR MATCHES "DEB" AND DPKGQ_CMD)
            # find the actual version, executes: dpkg-query
            # --showformat='\${Version}'
            # --show libusb-1.0-0
            execute_process(COMMAND "${DPKGQ_CMD}" --showformat='\${Version}'
                                    --show "${match}" OUTPUT_VARIABLE DPKGQ_VER)
            ...
            set(CPACK_DEBIAN_PACKAGE_DEPENDS "${CPACK_DEBIAN_PACKAGE_DEPENDS}"
                                             "${TEMP_TEST} (>= ${DPKGQ_VER}), ")
        elseif (CPACK_GENERATOR MATCHES "RPM" AND RPM_CMD)
            # find the actual version, executes: rpm -qp --queryformat
            # '%{VERSION}' libusb1
            execute_process(COMMAND "${RPM_CMD}" -q --queryformat '%{VERSION}'
                                    ${match} OUTPUT_VARIABLE RPM_VER)
        ...
        if (CPACK_GENERATOR MATCHES "DEB" AND DPKG_CMD)
            ...
            execute_process(COMMAND "${DPKG_CMD}" -L "${match}"
...
configure_file("${CMAKE_SOURCE_DIR}/cmake/add_requirements2tar.sh.in"
               "${CMAKE_BINARY_DIR}/add_requirements2tar.sh" IMMEDIATE @ONLY)
add_custom_target(
    required2tar
