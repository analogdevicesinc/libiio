...
set(CMAKE_INSTALL_DOCDIR
    ...
    CACHE
        PATH
        "documentation root (DATAROOTDIR/doc/${PROJECT_NAME}${LIBIIO_VERSION_MAJOR}-doc)"
)
...
set(CMAKE_INSTALL_DOCDIR
    "${CMAKE_INSTALL_DATAROOTDIR}/doc/${PROJECT_NAME}${LIBIIO_VERSION_MAJOR}-doc"
)
...
if (${CMAKE_SYSTEM_NAME} MATCHES "Darwin" AND OSX_FRAMEWORK)
    ...
    add_custom_command(
        ...
        COMMENT "Fixup Current symbolic link"
        VERBATIM)
else ()
    set(IIO_TESTS_INSTALL_DIR ${CMAKE_INSTALL_BINDIR})
endif ()

configure_file(artifact_manifest.txt.cmakein
               ${CMAKE_CURRENT_BINARY_DIR}/artifact_manifest.txt @ONLY)
...
if (NOT SKIP_INSTALL_ALL)
    ...
    if (WITH_UTILS)
        # install(TARGETS ${IIO_TESTS_TARGETS} RUNTIME DESTINATION
        # ${IIO_TESTS_INSTALL_DIR})

        # Workaround for CMake < 3.13, which do not support installing targets
        # built outside the current directory.
...
if (WITH_DOC)
    ...
    # It is not an error when 'dot' is not found, just switching off the
    # Doxygen's HAVE_DOT option
    ...
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/bindings/csharp/Doxyfile.in
                   ${CMAKE_CURRENT_BINARY_DIR}/Doxyfile_csharp @ONLY)
    configure_file(
        ${CMAKE_CURRENT_SOURCE_DIR}/CI/azure/generateDocumentationAndDeploy.sh.in
        ${CMAKE_CURRENT_BINARY_DIR}/generateDocumentationAndDeploy.sh
        @ONLY)
    file(
        COPY ${CMAKE_CURRENT_SOURCE_DIR}/doc
        DESTINATION ${CMAKE_HTML_DEST_DIR}/${CMAKE_API_DEST_DIR}
        FILES_MATCHING
        PATTERN "*.svg")
    file(COPY ${CMAKE_CURRENT_SOURCE_DIR}/doc/html/
         DESTINATION ${CMAKE_HTML_DEST_DIR})
    set(IIO_TESTS_MAN_PAGES_HTML "")
    foreach (_page ${IIO_TESTS_TARGETS})
        set(IIO_TESTS_MAN_PAGES_HTML
            "${IIO_TESTS_MAN_PAGES_HTML}<li><a href=\"./man1/${_page}.1.html\">${_page}</a></li>"
        )
    endforeach ()
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/doc/index.html.in
    ...
    add_custom_command(
        ...
        COMMAND ${DOXYGEN_EXECUTABLE}
                ${CMAKE_CURRENT_BINARY_DIR}/Doxyfile_csharp
        WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
...
if (OSX_PACKAGE)
    ...
    if (PKGBUILD_EXECUTABLE AND PRODUCTBUILD_EXECUTABLE)
        ...
        message(
            WARNING
                "Missing pkgbuild or productbuild: OSX installer won't be created."
        )
    endif ()
endif ()

if (NOT OSX_PACKAGE)
    # Support creating some basic binpkgs via `make package`. Disabled if
    # OSX_PACKAGE is enabled, as tarballs would end up empty otherwise.
    option(ENABLE_PACKAGING
           "Create .deb/.rpm or .tar.gz packages via 'make package'" OFF)
...
if (WITH_USB_BACKEND AND CMAKE_SYSTEM_NAME MATCHES "^Linux")
    iio_option(INSTALL_UDEV_RULE
               "Install a udev rule for detection of USB devices" ON "POSIX")

    if (INSTALL_UDEV_RULE)
        set(UDEV_RULES_INSTALL_DIR
            /lib/udev/rules.d
            CACHE PATH "default install path for udev rules")

        configure_file(libiio.rules.cmakein
                       ${CMAKE_CURRENT_BINARY_DIR}/90-libiio.rules @ONLY)
        install(FILES ${CMAKE_CURRENT_BINARY_DIR}/90-libiio.rules
...
if (WITH_SERIAL_BACKEND_DYNAMIC)
    install(TARGETS iio-serial
            LIBRARY DESTINATION ${CMAKE_INSTALL_LIBDIR}/libiio)
endif ()
