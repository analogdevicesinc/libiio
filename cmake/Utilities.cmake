...
function(collect_all_compile_defs_and_opts tgt out_defs out_opts)
	...
	set(all_opts "")
	set(_visited "")
	_get_recursive_interface_properties(${tgt} "${tgt_source_dir}" all_defs all_opts _visited)
...
function(_get_recursive_interface_properties tgt tgt_source_dir out_defs out_opts _visited)
	...
	foreach(dep IN LISTS _deps)
		unset(dep_src_dir)
		if(TARGET ${dep})
			get_target_property(interface_defs ${dep} INTERFACE_COMPILE_DEFINITIONS)
			...
			_get_recursive_interface_properties(${dep} "${dep_src_dir}" dep_defs dep_opts "${_visited}")
...
function(collect_sources tgt out_list)
	...
	foreach(src IN LISTS tgt_sources)
		...
		foreach(lib IN LISTS linked_libs)
			if(TARGET ${lib})
				get_target_property(lib_iface_inc ${lib} INTERFACE_INCLUDE_DIRECTORIES)
				if(lib_iface_inc)
		...
		if (MSVC AND NOT EXISTS "${CMAKE_BINARY_DIR}/run_cl_with_env.bat")
			...
			if(VS_PATH)
				...
				if(EXISTS "${VSDEV_CMD}")
					...
					file(WRITE "${CMAKE_BINARY_DIR}/run_cl_with_env.bat"
						"@echo off\n"
						"call \"${VSDEV_CMD}\" -arch=x64 >nul 2>&1\n"
						"\"${preproc_compiler}\" %*\n")
				else ()
					message(FATAL_ERROR "VsDevCmd.bat not found at expected location")
				endif()
			else()
				message(FATAL_ERROR "Visual Studio installation not found via vswhere")
			endif()
		...
		execute_process(
			COMMAND ${preproc_compiler} ${pp_flags} ${include_trace_flag} ${ALL_OPTS} ${include_flags} ${def_flags}
							${src_flags} ${sdk_flag} ${full_path} ${preproc_flags} ${opt_flags} ${std_flag}
		...
		foreach(raw IN LISTS includes)
			...
			if(clean_path MATCHES "^${CMAKE_SOURCE_DIR}" AND NOT clean_path MATCHES "^${CMAKE_BINARY_DIR}")
				# Avoid duplicates and check existence
				if(EXISTS "${clean_path}" AND NOT clean_path IN_LIST seen_headers)
