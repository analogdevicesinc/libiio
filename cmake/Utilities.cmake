...
#
# format with this before committing:
#
# cmake-format --use-tabchars --fractional-tab-policy use-space --line-width 120
...
# Usage: Collects all compile definitions and options for a target, including recursively from dependencies, and also
# includes directory/global properties at each step. collect_all_compile_defs_and_opts(my_target ALL_DEFS ALL_OPTS)
...
function (collect_all_compile_defs_and_opts tgt out_defs out_opts)
    ...
    set(all_opts "")
    set(_visited "")
    _get_recursive_interface_properties(${tgt} "${tgt_source_dir}" all_defs
                                        all_opts _visited)
...
function (_collect_direct_properties tgt tgt_source_dir defs_out opts_out)
    ...
    set(_defs "")
    foreach (def IN LISTS private_defs interface_defs directory_defs
                          global_defs)
        if ("${def}" MATCHES "\\$<[^>]*>")
            continue()
        endif ()
...
function (_get_recursive_interface_properties tgt tgt_source_dir out_defs
          out_opts _visited)
    ...
    _collect_direct_properties(${tgt} "${tgt_source_dir}" direct_defs
                               direct_opts)
    ...
    if (NOT _deps OR _deps MATCHES "-NOTFOUND")
        # message(STATUS "No dependencies found for target: ${tgt}") Set outputs in parent scope!
        set(${out_defs}
            "${accum_defs}"
            PARENT_SCOPE)
        set(${out_opts}
    ...
    foreach (dep IN LISTS _deps)
        ...
        if (TARGET ${dep})
            ...
            # message(STATUS "  INTERFACE_COMPILE_DEFINITIONS for dep ${dep}: ${interface_defs}")
            ...
            _get_recursive_interface_properties(${dep} "${dep_src_dir}"
                                                dep_defs dep_opts "${_visited}")
...
# - Avoids re-preprocessing the same file more than once
# - Honors target’s include dirs and compile definitions
# - Skips system headers, only keeps ones under ${CMAKE_SOURCE_DIR}
# -----------------------------------------------------------------------------
function (collect_sources tgt out_list)
    ...
    foreach (src IN LISTS tgt_sources)
        ...
        set(all_includes "")
        get_target_property(public_includes ${tgt}
        ...
        foreach (lib IN LISTS linked_libs)
            if (TARGET ${lib})
                ...
                if (NOT lib_type STREQUAL "INTERFACE_LIBRARY")
                    get_target_property(lib_priv_inc ${lib} INCLUDE_DIRECTORIES)
                    if (lib_priv_inc)
                        list(APPEND all_includes ${lib_priv_inc})
                    endif ()
                endif ()
            endif ()
        endforeach ()
        ...
        if (APPLE OR MSVC)
            list(APPEND all_includes "${CMAKE_BINARY_DIR}")
            list(APPEND all_includes "${CMAKE_SOURCE_DIR}/include")
        endif ()
        ...
        foreach (dir IN LISTS filtered_includes)
            if (NOT dir MATCHES "-NOTFOUND")
                # Remove trailing slash (but keep "/" intact)
                if (NOT dir STREQUAL "/")
                ...
                list(FIND include_flags "${flag}" _index)
                if (_index EQUAL -1)
                    list(APPEND include_flags "${flag}")
                endif ()
            endif ()
        endforeach ()
        ...
        message(
            STATUS
                "cmd : ${preproc_compiler} ${pp_flags} ${include_trace_flag} ${ALL_OPTS} ${include_flags_str} ${def_flags_str} ${src_flags} ${sdk_flag} ${full_path} ${preproc_flags} ${opt_flags_str} ${std_flag}"
        )
        ...
        if (MSVC AND NOT EXISTS "${CMAKE_BINARY_DIR}/run_cl_with_env.bat")
            ...
            if (VS_PATH)
                ...
                if (EXISTS "${VSDEV_CMD}")
                    message(STATUS "Using ${VSDEV_CMD}")
                    file(
                        WRITE "${CMAKE_BINARY_DIR}/run_cl_with_env.bat"
                        "@echo off\n"
                        "call \"${VSDEV_CMD}\" -arch=x64 >nul 2>&1\n"
                        "\"${preproc_compiler}\" %*\n")
                else ()
                    message(
                        FATAL_ERROR
                            "VsDevCmd.bat not found at expected location")
                endif ()
            else ()
                message(
                    FATAL_ERROR
                        "Visual Studio installation not found via vswhere")
            endif ()

        endif ()
        if (MSVC)
            set(preproc_compiler "${CMAKE_BINARY_DIR}/run_cl_with_env.bat")
        endif ()
        execute_process(
            COMMAND
                ${preproc_compiler} ${pp_flags} ${include_trace_flag}
                ${ALL_OPTS} ${include_flags} ${def_flags} ${src_flags}
                ${sdk_flag} ${full_path} ${preproc_flags} ${opt_flags}
                ${std_flag}
...
# This provides a CMake function to check for SPDX-License-Identifier headers in the source files associated with a
# specified CMake target.
#
function (spdx_check_target target)
    ...
    foreach (abs_source_file IN LISTS all_sources)
        ...
        if (cached_license)
            ...
            string(FIND "${file_content}" "SPDX-License-Identifier:"
                        license_pos)
            ...
            string(SUBSTRING "${file_content}" ${license_pos} 100
                             license_segment)
    ...
    file(RELATIVE_PATH relative_target_dir "${CMAKE_SOURCE_DIR}"
         "${target_source_dir}")
    message(
        STATUS
            "=== License Check for ${relative_target_dir}: ${target} =========="
    )
