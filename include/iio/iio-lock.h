...
__api int iio_cond_wait(struct iio_cond *cond, struct iio_mutex *lock, unsigned int timeout_ms);
__api void iio_cond_signal(struct iio_cond *cond);

__api struct iio_thrd *iio_thrd_create(int (*thrd)(void *), void *d, const char *name);
__api int iio_thrd_join_and_destroy(struct iio_thrd *thrd);

__api struct iio_task *iio_task_create(
		int (*task)(void *firstarg, void *d), void *firstarg, const char *name);
