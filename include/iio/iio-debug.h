...
#define dev_err(dev, ...)                                                    \
	ctx_err(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__dev_id_or_null(dev), __SKIPFIRST(__VA_ARGS__, ""))
#define dev_warn(dev, ...)                                                    \
	ctx_warn(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__dev_id_or_null(dev), __SKIPFIRST(__VA_ARGS__, ""))
#define dev_info(dev, ...)                                                    \
	ctx_info(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__dev_id_or_null(dev), __SKIPFIRST(__VA_ARGS__, ""))
#define dev_dbg(dev, ...)                                                    \
	ctx_dbg(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__dev_id_or_null(dev), __SKIPFIRST(__VA_ARGS__, ""))

#define chn_err(dev, ...)                                                    \
	dev_err(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__chn_id_or_null(chn), __SKIPFIRST(__VA_ARGS__, ""))
#define chn_warn(dev, ...)                                                    \
	dev_warn(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__chn_id_or_null(chn), __SKIPFIRST(__VA_ARGS__, ""))
#define chn_info(dev, ...)                                                    \
	dev_info(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__chn_id_or_null(chn), __SKIPFIRST(__VA_ARGS__, ""))
#define chn_dbg(dev, ...)                                                    \
	dev_dbg(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__chn_id_or_null(chn), __SKIPFIRST(__VA_ARGS__, ""))
