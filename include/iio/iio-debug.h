...
__api __iio_printf void iio_prm_printf(const struct iio_context_params *params,
		enum iio_log_level msg_level, const char *fmt, ...);
...
/* Passing a null pointer for a "%s" directive is undefined behaviour; glibc
 * prints "(null)" but other C libraries are free to crash. Substitute the
 * string itself so that the device/channel ID is always printable. */
#define __dev_id_str(dev) ((dev) ? iio_device_get_id(dev) : "(null)")
#define __chn_id_str(chn) ((chn) ? iio_channel_get_id(chn) : "(null)")
...
#define dev_err(dev, ...) \
	ctx_err(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __dev_id_str(dev), \
			__SKIPFIRST(__VA_ARGS__, ""))
#define dev_warn(dev, ...) \
	ctx_warn(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __dev_id_str(dev), \
			__SKIPFIRST(__VA_ARGS__, ""))
#define dev_info(dev, ...) \
	ctx_info(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __dev_id_str(dev), \
			__SKIPFIRST(__VA_ARGS__, ""))
#define dev_dbg(dev, ...) \
	ctx_dbg(__dev_ctx_or_null(dev), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __dev_id_str(dev), \
			__SKIPFIRST(__VA_ARGS__, ""))

#define chn_err(dev, ...) \
	dev_err(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __chn_id_str(chn), \
			__SKIPFIRST(__VA_ARGS__, ""))
#define chn_warn(dev, ...) \
	dev_warn(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __chn_id_str(chn), \
			__SKIPFIRST(__VA_ARGS__, ""))
#define chn_info(dev, ...) \
	dev_info(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __chn_id_str(chn), \
			__SKIPFIRST(__VA_ARGS__, ""))
#define chn_dbg(dev, ...) \
	dev_dbg(__chn_dev_or_null(chn), "%s: " __FIRST(__VA_ARGS__, 0) "%s", __chn_id_str(chn), \
			__SKIPFIRST(__VA_ARGS__, ""))

#define prm_perror(params, err, ...) \
	do { \
		if (LIBIIO_MAX_LOG_LEVEL >= LEVEL_ERROR) { \
			char _buf[1024]; \
			int _err = -(err); \
...
#define dev_perror(dev, err, ...) \
	ctx_perror(__dev_ctx_or_null(dev), err, "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__dev_id_str(dev), __SKIPFIRST(__VA_ARGS__, ""))
#define chn_perror(dev, err, ...) \
	dev_perror(__chn_dev_or_null(chn), err, "%s: " __FIRST(__VA_ARGS__, 0) "%s", \
			__chn_id_str(chn), __SKIPFIRST(__VA_ARGS__, ""))
