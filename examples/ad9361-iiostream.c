...
static void wr_ch_str(
		struct iio_channel *chn, const char *what, const char *str)
{
		...
		struct iio_device *dev, int chid, struct iio_channel **chn)
{
	*chn = iio_device_find_channel(
			dev, get_ch_name("voltage", chid), d == TX);
	if (!*chn)
		*chn = iio_device_find_channel(
				dev, get_ch_name("altvoltage", chid), d == TX);
	return *chn != NULL;
}
...
int main(int argc, char **argv)
{
	...
	if (argc == 1) {
		...
		ASSERT((ctx = iio_create_context_from_uri(argv[1])) &&
				"No context");
	}
	...
	ASSERT(cfg_ad9361_streaming_ch(ctx, &rxcfg, RX, 0) &&
			"RX port 0 not found");
	ASSERT(cfg_ad9361_streaming_ch(ctx, &txcfg, TX, 0) &&
			"TX port 0 not found");

	printf("* Initializing AD9361 IIO streaming channels\n");
	ASSERT(get_ad9361_stream_ch(ctx, RX, rx, 0, &rx0_i) &&
			"RX chan i not found");
	ASSERT(get_ad9361_stream_ch(ctx, RX, rx, 1, &rx0_q) &&
			"RX chan q not found");
	...
	while (!stop) {
		...
		for (p_dat = (char *)iio_buffer_first(txbuf, tx0_i);
				p_dat < p_end; p_dat += p_inc) {
