...
static bool get_ad9371_stream_dev(
		struct iio_context *ctx, enum iodev d, struct iio_device **dev)
{
	...
	case TX:
		*dev = iio_context_find_device(ctx, "axi-ad9371-tx-hpc");
		return *dev != NULL;
	case RX:
		*dev = iio_context_find_device(ctx, "axi-ad9371-rx-hpc");
		return *dev != NULL;
	default:
		ASSERT(0);
		return false;
...
static bool get_ad9371_stream_ch(struct iio_context *ctx, enum iodev d,
		struct iio_device *dev, int chid, char modify,
		struct iio_channel **chn)
{
	*chn = iio_device_find_channel(dev,
	...
	if (!*chn)
		*chn = iio_device_find_channel(dev,
				modify ? get_ch_name_mod("voltage", chid,
							 modify)
				       : get_ch_name("voltage", chid),
				d == TX);
	return *chn != NULL;
		...
		struct iio_channel **chn)
{
	switch (d) {
	case RX:
		*chn = iio_device_find_channel(get_ad9371_phy(ctx),
...
static bool get_lo_chan(
		struct iio_context *ctx, enum iodev d, struct iio_channel **chn)
{
	switch (d) {
...
bool cfg_ad9371_streaming_ch(struct iio_context *ctx, struct stream_cfg *cfg,
		enum iodev type, int chid)
{
	...
	wr_ch_lli(chn, type == TX ? "TX_LO_frequency" : "RX_LO_frequency",
			cfg->lo_hz);
	return true;
}
...
int main(int argc, char **argv)
{
	...
	ASSERT(cfg_ad9371_streaming_ch(ctx, &rxcfg, RX, 0) &&
			"RX port 0 not found");
	ASSERT(cfg_ad9371_streaming_ch(ctx, &txcfg, TX, 0) &&
			"TX port 0 not found");

	printf("* Initializing AD9371 IIO streaming channels\n");
	ASSERT(get_ad9371_stream_ch(ctx, RX, rx, 0, 'i', &rx0_i) &&
			"RX chan i not found");
	ASSERT(get_ad9371_stream_ch(ctx, RX, rx, 0, 'q', &rx0_q) &&
			"RX chan q not found");
	ASSERT(get_ad9371_stream_ch(ctx, TX, tx, 0, 0, &tx0_i) &&
			"TX chan i not found");
	ASSERT(get_ad9371_stream_ch(ctx, TX, tx, 1, 0, &tx0_q) &&
			"TX chan q not found");
	...
	while (!stop) {
		...
		for (p_dat = iio_buffer_first(txbuf, tx0_i); p_dat < p_end;
				p_dat += p_inc) {
