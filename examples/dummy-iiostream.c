...
static ssize_t sample_cb(const struct iio_channel *chn, void *src, size_t bytes,
			 void *d)
{
		...
		case CHANNEL_READ:
			for (int i = 0; i < channel_count; ++i) {
				...
				if (buffer_read_method == CHANNEL_READ_RAW)
					...
					bytes = iio_channel_read(
							channels[i], rxbuf, buf,
							sample_size * buffer_length);

				printf("%s ", iio_channel_get_id(channels[i]));
				for (int sample = 0;
				     sample < bytes / sample_size; ++sample) {
					for (int j = 0; j < repeat; ++j) {
						if (fmt->length / 8 ==
						    sizeof(int16_t))
							printf("%" PRIi16 " ",
