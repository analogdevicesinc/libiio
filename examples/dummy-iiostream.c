...
{
	unsigned int repeat = iio_channel_get_data_format_repeat(chn) ?
			      iio_channel_get_data_format_repeat(chn) : 1;
	...
	while (!stop)
	{
		...
		case BUFFER_POINTER:
			for (int i = 0; i < channel_count; ++i) {
				...
				unsigned int repeat = iio_channel_get_data_format_repeat(channels[i]) ?
						      iio_channel_get_data_format_repeat(channels[i]) : 1;
