...
static ssize_t serial_write_data(
		struct iiod_client_pdata *io_data, const char *data, size_t len, int timeout_ms)
{
	struct iio_context_pdata *pdata = (struct iio_context_pdata *)io_data;
	enum sp_return sp_ret;
	ssize_t ret;

	sp_ret = sp_blocking_write(
			pdata->port, data, len, timeout_ms > 0 ? (unsigned int)timeout_ms : 0);
...
static ssize_t serial_read_data(
		struct iiod_client_pdata *io_data, char *buf, size_t len, int timeout_ms)
{
	struct iio_context_pdata *pdata = (struct iio_context_pdata *)io_data;
...
static struct iio_buffer_pdata *serial_open_buffer(
		const struct iio_device *dev, unsigned int idx, struct iio_channels_mask *mask)
{
	...
	buf->pdata = iiod_client_open_buffer(
			pdata->iiod_client, pdata->iiod_client, dev, idx, mask);
...
static int serial_reg_read(const struct iio_device *dev, uint32_t address, uint32_t *value)
{
	const struct iio_context *ctx = iio_device_get_context(dev);
	struct iio_context_pdata *pdata = iio_context_get_pdata(ctx);
...
static int serial_reg_write(const struct iio_device *dev, uint32_t address, uint32_t value)
{
	const struct iio_context *ctx = iio_device_get_context(dev);
	struct iio_context_pdata *pdata = iio_context_get_pdata(ctx);
	struct iiod_client *client = pdata->iiod_client;
...
static int serial_refresh_format(const struct iio_channel *chn)
{
	...
	ret = iiod_client_refresh_format(
			pdata->iiod_client, dev, chn, format_str, sizeof(format_str));
	if (ret < 0)
		return (int)ret;
		...
		enum sp_parity parity, enum sp_flowcontrol flow)
{
	...
	ctx = iiod_client_create_context(pdata->iiod_client, &iio_serial_backend, NULL, ctx_params,
			ctx_params_values, IIO_ARRAY_SIZE(ctx_params));
	ret = iio_err(ctx);
	if (ret)
