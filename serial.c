			      ...
			      char git_tag[8])
{
	...
	return iiod_client_get_version(pdata->iiod_client, NULL, major, minor,
				       git_tag);
}

static int serial_open(const struct iio_device *dev, size_t samples_count,
		       bool cyclic)
{
			   ...
			   uint32_t *mask, size_t words)
{
	...
	ret = iiod_client_read_unlocked(pdata->iiod_client, NULL, dev, dst, len,
					mask, words);
...
static ssize_t serial_write(const struct iio_device *dev, const void *src,
			    size_t len)
{
	const struct iio_context *ctx = iio_device_get_context(dev);
	struct iio_context_pdata *pdata = ctx->pdata;
	...
	iio_mutex_lock(pdata->lock);
	ret = iiod_client_write_unlocked(pdata->iiod_client, NULL, dev, src,
					 len);
	iio_mutex_unlock(pdata->lock);
