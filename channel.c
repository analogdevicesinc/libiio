...
static char * get_scan_element(const struct iio_channel *chn, size_t *length)
{
	...
	if (chn->repeat > 1)
		snprintf(repeat, sizeof(repeat), "X%u", chn->repeat);
...
unsigned int iio_channel_get_data_format_repeat(
		const struct iio_channel *chn)
{
	return chn->repeat;
}
		...
		void *dst, const void *src)
{
	...
	ptrdiff_t end = len * (chn->repeat ? chn->repeat : 1);
