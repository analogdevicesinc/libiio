...
static ssize_t writefd_io(struct parser_pdata *pdata, const void *src,
			  size_t len)
{
	ssize_t ret;
	struct pollfd pfd[2];
...
static ssize_t receive_data(struct DevEntry *dev, struct ThdEntry *thd)
{
	...
	if (dev->sample_size == thd->sample_size) {
		...
		return iio_buffer_foreach_sample(dev->buf, receive_sample,
						 &info);
	}
}
			 ...
			 unsigned int nb, bool is_write)
{
	...
	while (thd->active) {
		ret = thd_entry_event_wait(thd, &entry->thdlist_lock,
					   pdata->fd_in);
		if (ret)
			break;
	}
		      ...
		      const char *attr, enum iio_attr_type type)
{
	...
	case IIO_ATTR_TYPE_DEBUG:
		ret = iio_device_debug_attr_read(dev, attr, buf,
						 sizeof(buf) - 1);
		break;
	case IIO_ATTR_TYPE_BUFFER:
		ret = iio_device_buffer_attr_read(dev, attr, buf,
						  sizeof(buf) - 1);
		break;
	default:
		ret = -EINVAL;
...
int set_buffers_count(struct parser_pdata *pdata, struct iio_device *dev,
		      long value)
{
	int ret = -EINVAL;
	...
	if (value >= 1)
		ret = iio_device_set_kernel_buffers_count(dev,
							  (unsigned int)value);
err_print_value:
	print_value(pdata, ret);
	return ret;
