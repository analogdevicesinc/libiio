...
static void __avahi_group_cb(AvahiEntryGroup *group, AvahiEntryGroupState state,
			     void *d)
{
...
static void __avahi_client_cb(AvahiClient *client, AvahiClientState state,
			      void *d)
{
	if (!client) {
...
static void create_services(AvahiClient *c)
{
	...
	ret = avahi_entry_group_add_service(avahi.group, AVAHI_IF_UNSPEC,
					    AVAHI_PROTO_UNSPEC, 0, avahi.name,
					    "_iio._tcp", NULL, NULL, IIOD_PORT,
					    NULL);
	if (ret < 0) {
