...
ssize_t iio_device_write_raw(const struct iio_device *dev, const void *src,
			     size_t len)
{
				  ...
				  const char *attr, const void *src, size_t len)
{
	if (dev->ctx->ops->write_device_attr)
		return dev->ctx->ops->write_device_attr(dev, attr, src, len,
							IIO_ATTR_TYPE_DEVICE);
	else
		return -ENOSYS;
}

ssize_t iio_device_attr_write(const struct iio_device *dev, const char *attr,
			      const char *src)
{
...
{
	return iio_device_buffer_attr_write_raw(dev, attr, src,
						strlen(src) + 1);
}
...
int iio_device_attr_read_bool(const struct iio_device *dev, const char *attr,
			      bool *val)
{
	long long value;
...
int iio_device_attr_read_double(const struct iio_device *dev, const char *attr,
				double *val)
{
	char buf[1024];
	ssize_t ret = iio_device_attr_read(dev, attr, buf, sizeof(buf));
...
int iio_device_attr_write_double(const struct iio_device *dev, const char *attr,
				 double val)
{
	ssize_t ret;
	char buf[1024];
...
int iio_device_attr_write_bool(const struct iio_device *dev, const char *attr,
			       bool val)
{
	ssize_t ret;
...
int iio_device_reg_write(struct iio_device *dev, uint32_t address,
			 uint32_t value)
{
	ssize_t ret;
	char buf[1024];
...
int iio_device_reg_read(struct iio_device *dev, uint32_t address,
			uint32_t *value)
{
	...
	long long val;
	int ret = iio_device_debug_attr_write_longlong(dev, "direct_reg_access",
						       (long long)address);
	if (ret < 0)
		return ret;

	ret = iio_device_debug_attr_read_longlong(dev, "direct_reg_access",
						  &val);
	if (!ret)
		*value = (uint32_t)val;
	return ret;
			  ...
			  void *data)
{
	...
	case IIO_ATTR_TYPE_BUFFER:
		...
		ret = (int)iio_device_buffer_attr_read(dev, NULL, buf,
						       0x100000);
		break;
	default:
		ret = -EINVAL;
			   ...
			   void *data)
{
	...
	case IIO_ATTR_TYPE_DEVICE:
		...
		break;
	case IIO_ATTR_TYPE_DEBUG:
		ret = (int)iio_device_debug_attr_write_raw(dev, NULL, buf,
							   ptr - buf);
		break;
	case IIO_ATTR_TYPE_BUFFER:
		ret = (int)iio_device_buffer_attr_write_raw(dev, NULL, buf,
							    ptr - buf);
		break;
	default:
		ret = -EINVAL;
