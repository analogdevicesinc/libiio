...
static int usb_open(const struct iio_device *dev, size_t samples_count,
		    bool cyclic)
{
...
static ssize_t usb_write(const struct iio_device *dev, const void *src,
			 size_t len)
{
	struct iio_device_pdata *pdata = dev->pdata;
	ssize_t ret;
...
static int usb_set_timeout(struct iio_context *ctx, unsigned int timeout)
{
	...
	ret = iiod_client_set_timeout(pdata->iiod_client, &pdata->io_ctx,
				      remote_timeout);
	if (!ret)
				 ...
				 unsigned int interface)
{
	...
	if (desc.iProduct == 0) {
		...
		ret = libusb_get_string_descriptor_ascii(
				hdl, desc.iProduct, (unsigned char *)product,
				sizeof(product));
		if (ret < 0)
			product[0] = '\0';
	}
